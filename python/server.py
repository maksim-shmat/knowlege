...
class AccessDenied(Fault):
    ...
    def __init__(self, message="Access denid"):
        super().__init__(ACCESS_DENIED, message)

def inside(dir, name):
    """Checks whether a give file name lies within a given direcrory."""
    dir = abspath(dir)
    name = abspath(name)
...
class Node:
    ...
    def query(self, query, history=[]):
        """
        Perform a query for a file, possibly asking other known Nodes for
        help. Return the file as a string.
        """
        try:
    ...
    def _start(self):
        """Used intrnally to start the XML-RPC server."""
        s = SimpleXMLRPCServer(('', get_port(self.url)), logRequest=False)
    ...
    def _handle(self, query):
        ...
        if not inside(dir, name): raise AccsessDenied
