...
class ViewOne(Toplevel):
    ...
    def sizeToDisplaySide(self, scaler):
        ...
        imgwide, imghigh = scaler(scrwide, scrhigh, imgwide, imghigh)
    ...
    def onSizeToDisplayWidth(self, event):
        def scaleWide(scrwide, scrhigh, imgwide, imghigh):
            newwide = scrwide
            newhigh = int(scrwide * (imghigh / imgwide)) # true division
            return (newwide, newhigh)
    ...
    def viewThumbs(imgdir, kind=Toplevel, numcols=None,
        ...
        # maximum width|height: thumb=(name, obj), thumb.size=(width, height)
        ...
        savephotos = []
        while thumbs:
            thumsrow, thumbs = thumbs[:numcols], thumbs[numcols:]
            colpos = 0
            for (imgfile, imgobj) in thumbsrow:
                ...
                def handler(savefile=imgfile):
                    ...
                    link.config(command=handler, width=linksize, hight=linksize)
