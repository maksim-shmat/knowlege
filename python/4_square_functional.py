"""4 squares puzzel in functional style."""
                ...
                if a > h:
                    ...
                    def right(e):
                        ...
                        if ((g > h) or (uniq and (g == c))):
                            ...
                            agDelta = a - g
                            bfs = difference(ns)([d, c, e, g, a]) if uniq else ns
                            # MID LEFT AND RIGHT :: b and f
                            def midLeftRight(b):
                                f = b + agDelta
                                return [[a, b, c, d, e, f, g]] if ((f in bfs) and ((not uniq) or (f not in [a, b, c, d, e, g]))) else []
    
    # CANDIDATE DIGITS BOUND TO POSITIONS [a .. g] -----------
                            return concatMap(midLeftRight)(bfs)
                    return concatMap(right)(difference(xs)([d, c, a]) if uniq else ns)
            return concatMap(left)(delete(d)(xs) if uniq else ns)
...
# main :: IO ()
def main():
    """Testing unique digits [1..7], [3..9] and unresticted digits"""
    print(main.__doc__ + ':\n')
    print(unlines(map(
        lambda tpl: '\nrings' + repr(tpl) + ':\n\n' + unlines(
...
# concatMap :: (a -> [b]) -> [a] -> [b]
def concatMap(f):
    """A conactenated list over wich a function has been mapped.
    The list monad can derived by using a function f which
    wraps its output in a list,
    (using an empty to represent computational failure).
    """
...
def uncurry(f):
    ...
    derived from a vanilla or current function.
    """
