"""Align columns with StringIO."""

from StringIO import StringIO

textinfile = """Given$a$txt$file$of$many$lines,$where$fields$within$a$line$
are$delineated$by$a$single$'dollar'$character,$write$a$program
that$align$each$column$of$fields$by$ensuring$that$words$in$each$
column$are$separated$by$at$least$one$space.
Further,$allow$for$each$word$in$a$least$one$space.
...
def aligner(infile, justification = 'L'):
    '''
    Justify columns of textual tabular input where the row separator is the
    newline and the field separator is a 'dollar' character.
    ...
    fieldsbyrow = [fields + ['']*(maxfield - len(fields))
            for fields in fieldsbyrow]

    # rotate
    fieldsbycolumn = zip(*fieldsbyrow)
    # calculate max fieldwidth per column
    colwidth = [max(len(field) for field in column)
            for column in fieldsbycolumn]
    # pad fields in column to colwidth with spaces
...
for align in 'Left Right Center'.split():
    infile = StrinIO(textinfile)
    print("\n %s Column-aligned output:" % align)
    print(aligner(infile, aling[0]))
