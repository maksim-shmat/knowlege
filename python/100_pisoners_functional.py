...
# all ChainedPathsAreShort :: Int -> IO (0/1)
...
# randomTialResult :: RandomIO (0/1) -> Int -> (0/1)
...
def Nothing():
    ...
    Empty wrapper returned where a cuputation is not possible.
    '''
...
def randomRInt(m):
    ...
    called, yields a new pleudo-random integer
    in the range [m..n].
    '''
...
def unfoldr(f):
    ...
    Where caamorphism reduces a list to a summary value,
    the anamorphic unfoldr builds a list from a seed value.
    As long as f returns Just(a, b), a is prepended to the list,
    and the residual b is used as the argument for the next
    application of f.
    When f returns Nothing, the competed list is returned.
    '''
    def go(v):
        xr = v, v
        xs = []
