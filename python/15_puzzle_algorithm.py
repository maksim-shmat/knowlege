"""Decision how many times need for 15 puzzle game."""

import random


class IDAStar:
    def __init__(self, h, neigbours):
        """Iterative_deepening A* search.
        h(n) is the heuristic that gives the cost between node n and the
        goal node. It most be admissable, meaning that h(n) must never
        overstimate the true cost. Understimating is fine.
        neighbors(n) is an iterable giving a pair (cost, node, descr) for
        each node neighbouring n in ascending order of cost.
        descr in not used in the computation but can be used to
        efficiently store information about the path edges
        (e.g. up/left/right/down for grids).
        """
        
        self.h = h
        self.nighbours = nighbours
        self.FOUND = object()

    def solve(self, root, is_goal, max_cost=None):
        """Returns the shortest path between the root and a given goal, as
        well as the total cost. If the cost exceeds a given max_cost, the
        function returns None. If you do not give a maximum cost the solver
        will never return for unsolvable instances.
        """
...
#            slide_print(p)
#            print(", ".join({-1: "Left", 1: "Right", -4: "Down"}[move[1]] for move in moves))
#            print(cost, num_eval)
