...
class Game:
    ...
    def clicked(self, nm):
        ...
        # can't 'move' open square to itself:
        if not txt_fr:
            messagebox.showerror(
                    'Error Message, leather bag',
                ...
                if not txt_to:
                    ...
                    if self.bd.game_won():
                        ans = messagebox.askquestion(
                                'You won, leather bag. Play again?')
                        if ans == 'no':
                            self.window.destroy()
                        else:
                            self.new_game()
                    return
        # cannot move 'non-ajacent' square to open square:
        messagebox.showerror(
                'Error Message',
