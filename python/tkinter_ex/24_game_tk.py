...
Combination of '24 game' and '24 game/Solve' allowing user or random selection
of 4-digit number and user or computer solution.
Note that all computer solution are displayed."""

from tkinter import *
from tkinter import messagebox
from tkinter.scrolledtext import ScrolledText
# from tkinter import scrolledtext in later version?
import random
import itertools
...
class Game:
    ...
    def get_digits(self):
        ...
        save of valid and switch screen to solution mode.
        '''
    ...
    def gen_digits(self):
        ...
        save them, and swith screen to solution mode.
        '''
        self.digit_ent.delete(0, 'end')
        self.digits = ''.join([random.choice('1234567890')
            for i in range(4)])
    ...
    def check_soln(self):
        ...
        for t in txt:
            ...
            if t in '+-*/':
                if dgt_op == 'o':
                    ...
                    self.err_msg('Need digit before operator')
                    return
        ...
        try:
            ...
            if round(eval(txt),5) == 24:
                ...
                self.show_soln()  # shown all solutions
                return
        except:
    ...
    def show_soln(self):
        ...
        # list of all (hopefully) expressions for 4 operands and 3 operations
        ...
        for fm in formats:                      # pick a format
            for c in combs:                     # pug in3 ops
                f = fm.replace('op1', c[0])
                f = f.replace('op2', c[1])
                f = f.replace('op3', c[2])
                for A, B, C, D in perms:        # evaluate expression
                    x = f.replace('A', A)
                    x = x.replace('B', B)
                    x = x.replace('C', C)
                    x = x.replace('D', D)
                    try:                        # evaluate expression
                        # round covers up Pythons's representation of float
                        if round(eval(x), 5) == 24:
                            lox.append(' ' + x)
                    except ZeroDivisionError:   # can innore these
                        continue
        if lox:
            txt = '\n'.join(x for x in lox)
        else:
