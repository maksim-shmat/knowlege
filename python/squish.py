"""The main game module (squish.py)."""
...
class Level(State):
    ...
    def update(self, game):
        ...
        # Otherwisef, if the weight has landed, reset it. If all the 
    ...
    def display(self, screen):
        """
        Display the state after the first display (whixh simply wipes
        the screen). As opposed to first Display, this method uses
...
class Paused(State):
    ...
    """
    finished = 0  # Has the use ended the pause?
...
class Info(Paused):
    ...
    trying to survive a course in self-defence against fruit,
    where the participants will "defend" themselves
    against you with a 16 ton weight."""


class StartUp(Paused):
    """
    A paused state that display a splash image and a welcome
...
class GameOver(Paused):
    """
    A state that informs the user that he or she has lost the game.
...
class Game:
    ...
    def __init_(self, *args):
        ...
        # Move th that directory (so that the image files may be
        # opened later on):
        os.chdir(dir)
        # Start with no state:
        self.state = None
