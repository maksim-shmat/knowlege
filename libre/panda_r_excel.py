...
# this_dir = Path(".").resolve()  # for current directory

# Read in all Excel files from all subfolders of sales_data

parts = []
for path in (this_dir / "sales_data").rglob("*.xls*"):  # recursive globing
    print(f'Reading {path.name}')
    part = pd.read_excel(path, indexf_col="transaction_id")
    parts.append(part)
...
summary.to_excel(this_dir / "sales.report_pandas.xlsx")
